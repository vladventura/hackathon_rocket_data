...
with open(f'data/{starting_year}/rocket_launch_data.csv', 'w') as csvfile:
    ...
    filewriter.writerow(['Date and Time', 'Rocket', 'Flight Number', 'Country', 'Launch Site', 'LSP', 'Payload', 'Operator', 'Orbit', 'Function', 'Decay', 'Outcome', 'Remarks'])
    data_list = []
    extra_list = []
    for tr in first_tbody.findAll('tr'):
        ...
        if monthSpan:
            monthSpanText = monthSpan.get_text() if monthSpan.get_text() != 'To be determined' else ""
            if monthSpanText:
                print(monthSpanText)
